...
vector<int> compute(vector<int> &nums) {
	...
	for (int i = 1, j = 0; i < n; i++) {
		while (j > 0 && nums[i] != nums[j]) j = pi[j - 1];
		if (nums[i] == nums[j]) j++;
		pi[i] = j;
	}
	return pi;
...
void solve(int test_case [[maybe_unused]]) {
	int N, M;
	cin >> N >> M;
	vector<int> bears(N), walls(M);
	for (int &d : bears) cin >> d;
	for (int &d : walls) cin >> d;
	...
	vector<int> nbears(N - 1), nwalls(M - 1);
	for (int i = 0; i < N - 1; i++) {
		nbears[i] = bears[i + 1] - bears[i];
	}
	for (int i = 0; i < M - 1; i++) {
		nwalls[i] = walls[i + 1] - walls[i];
	}

	int ans = 0;
	vector<int> pi = compute(nwalls);
	for (int i = 0, j = 0; i < N - 1; i++) {
		while (j > 0 && nbears[i] != nwalls[j]) j = pi[j - 1];
		if (nbears[i] == nwalls[j]) j++;
		if (j == M - 1) {
			ans++;
			j = pi[j - 1];
		}
